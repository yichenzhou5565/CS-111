...
void inode(unsigned int inode_table, unsigned int inode_num, unsigned int inode_index)
{
    ...
    unsigned long offset = 1024 + (inode_table) * block_size + inode_index*sizeof(inode);
        ...
        for (k = 0; k < block_size/4; k++){
            ...
            if(indir1 != 0) {
                for (j = 0; j < block_size/4; j++) {
                    ...
                    if (indir2 != 0){
                        int byte_offset = 0;
                        unsigned long offset = indir2 * block_size;
                        while (byte_offset < block_size) {
                            pread(fd, &directory, sizeof(struct ext2_dir_entry), offset + byte_offset);
                            if(directory.inode == 0){
                                byte_offset += directory.rec_len;
...
void triple_indirect(unsigned int inode_num, unsigned int indir_block) {
    ...
    for(i = 0; i < block_size/4; i++){
        ...
        if(indir1 != 0){
            fprintf(stdout, "INDIRECT,%d,%d,%d,%d,%d\n", inode_num, 3, 12 + block_size/4 + block_size/4 * block_size/4 + i * block_size/4 * block_size/4, indir_block, indir1);
            unsigned long offset2 = indir1 * block_size;
            for(j = 0; j < block_size/4; j++){
                pread(fd, &indir1, 4, offset2 + j*4);
                if(indir2 != 0){
                    fprintf(stdout, "INDIRECT,%d,%d,%d,%d,%d\n", inode_num, 2, 12 + block_size/4 + block_size/4 * block_size/4 + i * block_size/4 * block_size/4 + j*block_size/4, indir1, indir2);
                    unsigned long offset3 = indir2 * block_size;
                    for(k = 0; k < block_size/4; k++){
                        pread(fd, &ref_block, 4, offset3 + k*4);
                        if (ref_block != 0) {
                            fprintf(stdout, "INDIRECT,%d,%d,%d,%d,%d\n", inode_num, 1, 12 + block_size/4 + block_size/4 * block_size/4 + i * block_size/4 * block_size/4 + j*block_size/4 +k, indir2, ref_block);
                        }
                    }
                }

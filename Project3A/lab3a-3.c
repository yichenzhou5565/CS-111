...
void inode(unsigned int inode_table, unsigned int inode_num, unsigned int inode_index)
{
    ...
    char ctime[32];
    get_time (inode_i_ctime, ctime);
    char mtime[32];
    get_time (inode_i_mtime, mtime);
    char atime[32];
    get_time (inode_i_atime, atime);
...
void directory_entry(unsigned int inode_num, unsigned int dir_block) {
    struct ext2_dir_entry directory;
    
    unsigned long offset = dir_block * block_size;
    int byte_offset = 0;
    while(byte_offset < block_size){
        pread(fd, directory, sizeof(struct ext2_dir_entry), offset + byte_offset)
        
        if(inode_num == 0){ //inode starts at 1, if inode num == 0, just update offset & skip
